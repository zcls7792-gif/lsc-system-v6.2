...
CREATE TABLE IF NOT EXISTS `lsc_accounts` (
    ...
    `total_frozen`    BIGINT      NOT NULL DEFAULT 0        COMMENT '冻结LSC总量(风控冻结池,V7.3新增)',
...
CREATE TABLE IF NOT EXISTS `lsc_transactions` (
    ...
    `type`              TINYINT(2)   NOT NULL              COMMENT '流水类型(V7.3): 1消费赠送入锁定 2订单抵扣 3退款退回 4退款回扣 5到期作废 6推荐奖励入锁定 7每日释放 8风控冻结 9风控解冻',
    ...
    `before_frozen`     BIGINT       NOT NULL DEFAULT 0   COMMENT '操作前冻结余额(V7.3新增)',
    `after_frozen`      BIGINT       NOT NULL DEFAULT 0   COMMENT '操作后冻结余额(V7.3新增)',
...
CREATE TABLE IF NOT EXISTS `available_lsc_details` (
    ...
    `status`               TINYINT(1)   NOT NULL DEFAULT 1  COMMENT '状态(V7.3): 1有效 2已作废 3已使用 4退款退回',
    `created_at`           DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at`           DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `products` (
    ...
    `cost_price`          DECIMAL(18,2) NOT NULL DEFAULT 0.00 COMMENT '成本价/进货价(元，V7.3用于计算进销差赠送LSC)',
    `grant_points`        BIGINT        NOT NULL DEFAULT 0    COMMENT '赠送LSC积分数量(V7.3按进销差自动计算，上限为售价100%)',
...
CREATE TABLE IF NOT EXISTS `orders` (
    ...
    `grant_points`       BIGINT        NOT NULL DEFAULT 0   COMMENT '单件商品赠送LSC积分快照(V7.3，下单时从商品快照)',
    `granted_lsc`        BIGINT        NOT NULL DEFAULT 0   COMMENT '本订单实际赠送LSC数量(V7.3，=grant_points×quantity×人民币支付比例，入锁定池)',
